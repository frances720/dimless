...
for i = 1:pt
    ...
    Dtstar = ((x-Lcstar(i)).*(1-D1star(i).^(1/3))./Ltstar(i) + 1).^3;
    Dstar = (x > Lcstar(i)).*D2star(i) + (x < Lcstar(i)-Ltstar(i)).*D1star(i) +...
        ((x <= Lcstar(i)) & (x >= (Lcstar(i)-Ltstar(i)))).*Dtstar(i);
    Dstar = Dstar';
